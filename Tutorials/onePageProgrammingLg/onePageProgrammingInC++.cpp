...
3.4 Function taking variable length arguments
	- void do(...);
3.5  Diff way of calling 
	call by value => f(int i, int j);
	call by ref   => f( int &i, int &j);
	call by ptr/addr => f(int *i, int *j);
3.6 Return a Value/ return a pinter / return a ref
...
7.1 Inheritence in C++
	...
	3.default :
	4. Private memner of base can't be derived.
...
7.3 Composition Aggrigation or nesting the classes
...
7.7 Interface and Abstract class in C++	 
