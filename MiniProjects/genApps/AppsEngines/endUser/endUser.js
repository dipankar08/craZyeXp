...
myApp.controller("UserController",  function ($scope,$http,$sce) { 
...
$scope.getMiniView=function(a) {
  $http.get("/api/user/?page="+a+"&limit="+$scope.limit+"")
      .success(function(data, status, headers, config) {
       ...
       // Not incduing this feature tableResize('#table_miniview_User');
      })
...
$scope.selectItem = function(a) {
    $http({
          method: "post",
          url: '/api/user/aq/?limit=50',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
          data:{'include':'name'}
    })
    .success(function(data, status, headers, config) {
    $scope.user_lookup = data.res.data;
    })
    .error(function(data, status, headers, config) {
    }); 
}
...
$scope.qsUser= function(a) {
     $http.get("/api/user/qs/?q="+$scope.quick_search.in)
    .success(function(data, status, headers, config) {
      console.log(data);
      $scope.quick_search.out= data.res;
      $scope.status = data.status; $scope.msg=data.msg
      //addClass('#m2m-user','show');
    })
    .error(function(data, status, headers, config) { console.log('Error happen with status:'+status)}); 
  }
