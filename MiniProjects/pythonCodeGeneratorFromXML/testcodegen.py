...
for model in models:
  ...
  for f in fields:
    ...
    if f.getAttribute('user_input') == 'yes':
      arg.append(fname)
    elif f.getAttribute('user_input') == 'ref':
      arg.append(fname)      
  ...
  # Generating api.py 
  aps+= "from .models import %s" %mname
  aps+= "class %sManager:"%mname; aps.indent()
  aps*="""
@staticmethod
...
def get{MODEL_NAME}(id):
  try:
    ...
    return {{'res':model_to_dict(t),'status':'info','msg':'{MODEL_NAME} returned'}}
  except Exception,e :
...
mf = open('autoGenEngine/models.py','w+');mf.write((ms.code));mf.close();
